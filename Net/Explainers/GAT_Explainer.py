...
def load_models_and_hparams(model_dir):
    ...
    pattern = r"loss_(\w+)_h(\d+)_b(\d+)_lr([0-9.]+)_d([0-9.]+)_hd(\d+)"
    match = re.search(pattern, folder_name)
    if not match:
        raise ValueError(f"Could not parse hyperparameters from: {folder_name}")

    loss_name, hidden, batch, lr, dropout, heads = match.groups()

    # Map strings to actual objects (expand as needed)
    loss_fn_map = {
        "MSELoss": torch.nn.MSELoss,
        "L1Loss": torch.nn.L1Loss,
        "SmoothL1Loss":torch.nn.SmoothL1Loss(beta = 0.5),
    }
...
class GATConv(torch.nn.Module):
    def __init__(self, input_dim, hidden_channels, dropout, heads):
        ...
        self.conv1 = GATv2Conv(input_dim, hidden_channels, heads=heads, concat=True)
        self.dropout = torch.nn.Dropout(dropout) 
        self.pooling_function = global_mean_pool
        self.out_layer = Linear(hidden_channels * heads, 1)
...
def process_fold(pth_file, hidden_channels, dropout, heads, device, data_pkl_path):
    ...
    model = GATConv(input_dim=input_dim, hidden_channels=hidden_channels, dropout=dropout, heads = heads).to(device)
...
if __name__ == "__main__":
    ...
    model_dir = "/home/ziyu-song/pKa/Results/GAT_Grid_Search/2/saved_models/dataset_4_3/loss_MSELoss_h48_b32_lr0.01_d0.5_hd4"
    ...
    data_pkl_path = "/home/ziyu-song/Graph_pKa/Data/4_Residues_W_Local_Frame/Subsets/data_list_3.pkl"
    ...
    mean_importance_df.to_csv('/home/ziyu-song/pKa/Results/GAT_Grid_Search/1/Feature_Importance/GAT_Feature_Importance_1.csv', index=False)
