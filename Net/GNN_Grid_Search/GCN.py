...
base_dir = "/home/ziyu-song/pKa/Results/GCN_Grid_Search"
DATASET_DIR = Path("/home/ziyu-song/Graph_pKa/Data/4_Residues_W_Local_Frame/Subsets")
...
class GCNNet(torch.nn.Module):
    def __init__(self, input_dim, hidden_channels, dropout):
        ...
        print(f"Initializing GCNLayer: input_dim={input_dim}")

        self.conv1 = GCNConv(input_dim, hidden_channels, add_self_loops=False)
        self.conv2 = GCNConv(hidden_channels, 1, add_self_loops=False)
...
def save_predictions_to_csv(all_best_predictions, dataset_idx, loss_function, hidden_channels, batch_size, patience, k_folds, lr, dropout):
    ...
    output_dir = f"{base_dir}/all_best_predictions/dataset_{dataset_idx}/{loss_function.__class__.__name__}_hidden_{hidden_channels}_bs_{batch_size}_patience_{patience}_kf_{k_folds}_lr_{lr}_dropout_{dropout}"
...
def train_and_evaluate(loss_function, hidden_channels, batch_size, patience, k_folds, lr, dropout, dataset_idx, data_list, input_dim, Residue_Type_labels):
    """
    This function trains and evaluates a GCN model using separate CPU cores.
    """
    device = torch.device("cpu")  # Force CPU usage for parallel training

    save_dir = f"{base_dir}/saved_models/dataset_{dataset_idx}/loss_{loss_function.__class__.__name__}_h{hidden_channels}_b{batch_size}_p{patience}_f{k_folds}_lr{lr}_d{dropout}"
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(data_list)):
        ...
        model = GCNNet(input_dim, hidden_channels, dropout).to(device)
        ...
        for epoch in range(500):  
            ...
            with torch.no_grad():
                ...
                avg_val_mae = total_abse / total_samples_val  # Ensure avg_val_loss is defined
                ...
                if avg_val_mae < best_MAE:
                    ...
                    counter += 1
                    if counter >= patience and epoch > 60:  # Ensure at least 10 epochs before stopping
                        # write_log(f"Thread-{thread_id} | Early stopping at epoch {epoch} (No improvement)")
                        print(f"Early stopping at epoch {epoch} (No improvement)")
                        break

                if epoch % 10 == 0:
                    print(f" Epoch {epoch}: Train Loss={avg_loss:.4f}, Val Loss={avg_val_mae:.4f}")
...
if __name__ == "__main__":
    ...
    # Display the best combination based on MAE (tie-break by RMSE)
    best_result = min(results, key=lambda x: (x[8], x[9]))
    print(
        f"\nBest Combination: Dataset={best_result[0]}, Loss_fn = {best_result[1]}, "
        f"Hidden={best_result[2]}, Batch={best_result[3]}, Patience={best_result[4]}, "
        f"K-Folds={best_result[5]}, LR={best_result[6]}, Dropout={best_result[7]}"
    )
    print(f"Best MAE: {best_result[8]:.4f}, Best RMSE: {best_result[9]:.4f}")

    # Save best result to a CSV
    best_result_df = pd.DataFrame([best_result],
                                  columns=['Dataset', 'Loss_fn', 'Hidden', 'Batch', 'Patience',
                                           'K-Folds', 'LR', 'Dropout', 'MAE', 'RMSE'])
    ...
    print("Results saved to 'grid_search_results_4_REsidues.csv' and 'best_result_4_REsidues.csv'")
