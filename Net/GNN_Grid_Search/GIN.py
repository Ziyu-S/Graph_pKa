...
base_dir = "/home/ziyu-song/pKa/Results/GIN_Grid_Search"
...
class GINLayer(torch.nn.Module):
    def __init__(self, input_dim, hidden_channels, dropout, pooling_function):
        ...
        print(f"Initializing GINLayer: input_dim={input_dim}")
...
def train_and_evaluate(loss_function, hidden_channels, batch_size, patience, k_folds, lr, dropout, pooling_function, dataset_idx, data_list, input_dim, Residue_Type_labels):
    """
    This function trains and evaluates a GIN model using separate CPU cores.
    """
    device = torch.device("cpu")  # Force CPU usage for parallel training

    save_dir = f"{base_dir}/saved_models/dataset_{dataset_idx}/loss_{loss_function.__class__.__name__}_h{hidden_channels}_b{batch_size}_p{patience}_f{k_folds}_lr{lr}_d{dropout}_pooling_fn_{pooling_function.__name__}"
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(data_list)):
        ...
        model = GINLayer(input_dim, hidden_channels, dropout, pooling_function).to(device)
...
if __name__ == "__main__":
    ...
    pooling_functions = [global_mean_pool, global_add_pool]
    ...
    for dataset_idx, data_list in enumerate(data_sets):
        ...
        param_combinations = list(product(loss_functions, hidden_channels_range, batch_size_range, patience_range, k_folds_range, learning_rate_range, dropout_range, pooling_functions))


        # Run training in parallel using multiple CPU cores
        dataset_results = Parallel(n_jobs=num_cores, )(
            delayed(train_and_evaluate)(*params, dataset_idx, data_list, input_dim, Residue_Type_labels) for params in param_combinations
        )
    ...
    print(f"\nBest Combination: Dataset={best_result[0]}, Loss_fn = {best_result[1]}, Hidden={best_result[2]}, Batch={best_result[3]}, "
        f"Patience={best_result[4]}, K-Folds={best_result[5]}, LR={best_result[6]}, Dropout={best_result[7]}, Pooling_fn = {best_result[8]}")
    ...
    best_result_df = pd.DataFrame([best_result], 
                                columns=['Dataset', 'Loss_fn', 'Hidden', 'Batch', 'Patience', 
                                        'K-Folds', 'LR', 'Dropout', 'Pooling_fn', 'MAE', 'RMSE'])
